...
laneDetection::laneDetection(): blockNum(9), windowSize(150), recordCounter(0), initRecordCount(0), failDetectFlag(true){}
...
void laneDetection::laneDetctAlgo()
{
    ...
    cv::cvtColor(mergeImage, mergeImageRGB, cv::COLOR_BGR2RGB);
    //else
    //    cv::cvtColor(mergeImage, mergeImageRGB, CV_BGR2RGB);
...
void laneDetection::laneFitting()
{
    ...
    if (CV_VERSION_MAJOR >= 4)
        polylines(maskImage, curveL, false, Scalar(255,0,0), 20, cv::LINE_AA);
    Mat curveR(curvePointsR, true);
    curveR.convertTo(curveR, CV_32S);
    if (CV_VERSION_MAJOR >= 4)
        polylines(maskImage, curveR, false, Scalar(0,0,255), 20, cv::LINE_AA);
